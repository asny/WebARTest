...
function createProduct(productInfo)
{
  ...
  // ANCHOR
  // Create anchor
  var posAnchor = localToWorld(position.clone().add(new THREE.Vector3(-0.2, 0.1, 0.05)));
...
function updateProducts(pos)
{
    ...
    for(var i = 0; i < products.length; i++)
    {
      ...
      for(var j = 0; j < product.certificates.length; j++)
      {
        ...
        if(shouldShow)
        {
          var t = (0.00005 * new Date().getTime() + ((1.0*j)/product.certificates.length) * 2.0 * Math.PI) % (2.0 * Math.PI);
